...
grazers$carbon.per.bite<-grazers$g.carbon.day/(grazers$bite.rate*60*12)
...
# UVC data load
load("../../data/wio_herb_benthic_merged.Rdata")
grazers<-pred %>% filter(FG == 'Herbivore Browser')
## add genera column
grazers$genus<-str_split_fixed(grazers$species, pattern = '\\ ', n=2)[,1]

## match bite rate for each species. 
grazers$bite.rate<-browser.bites$median[match(grazers$species, browser.bites$class)]
## if species is missing, use genera
grazers$bite.rate[is.na(grazers$bite.rate)]<-
          browser.bites$median[match(grazers$genus[is.na(grazers$bite.rate)], browser.bites$class)]
## if genera is missing, use global mean
grazers$bite.rate[is.na(grazers$bite.rate)]<-browser.bites$median[browser.bites$preds == 'global.mean']
