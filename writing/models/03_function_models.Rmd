...
h <- grazers %>% 
  ...
  ## mean cropping across transects at each site
          group_by(dataset, date, reef, site, management, unique.id, depth, FG,
                         hard.coral, macroalgae,  rubble, substrate, complexity, fish.biom) %>%
          summarise(cropping = mean(cropping), biom=mean(biom)) 
...
ggplot(h, aes(log10(cropping), log10(biom), col=dataset)) +geom_point() + ylab('Biomass (log10 kg/ha)') + xlab('log10 kg carbon removed per day') 
```