...
```{r, include = T, message=F, echo=F}
# UVC data load
load("../../data/wio_herb_benthic_merged.Rdata"); scrapers.uvc<-scrapers
scrapers.uvc<-pred %>% filter(FG == 'Herbivore Scraper')
scrapers.uvc$species<-as.factor(scrapers.uvc$species)
...
## assign global means or genus START HERE
scraper.bites<-read.csv(file = '../../results/functions/scraper_bites_predicted.csv')
scrapers.uvc$biterate[is.na(scrapers.uvc$biterate),])

## area predictions for body size
d.pred <-data.frame(TL=scrapers.uvc$TL)
...
## how many global means in analysis?
t<-data.frame(table(scrapers.uvc$bite.rate))
...
## how much biomass is global mean in analysis?
t<-aggregate(biomass.kgha ~ bite.rate, scrapers.uvc, sum)
...
scrapers.uvc$scraping<-scrapers.uvc$biterate * scrapers.uvc$bitearea
