...
```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.cap='Correlation between unscaled explanatory covariates', fig.height=6, fig.width=6}
...
```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.cap='Correlation between scaled explanatory covariates', fig.height=4, fig.width=4}
...
m.grazer<-lmer(grazerlog10 ~ hard.coral + macroalgae * complexity + fish.dummy + pristine.dummy + ## fixed
          (1 | dataset/reef) , ## random, nested = reefs within datasets
                data = h)

```

```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.cap='Grazer model diagnostics - residuals and fitted values'}
## model diagnostic plots - grazer biomass
par(mfrow=c(2,2), mar=c(5,4,2,2))
hist(resid(m.grazer), main = 'model residuals', xlab='Residuals', cex.main=0.8)
hist(h$grazerlog10, main = 'grazer biomass distribution', xlab='Biomass', cex.main=0.8)
...
nd$macroalgae <- 0; nd$complexity <-0
...
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$hard.coral <- 0; nd$complexity <-0
p.algae<-predict(object=m.grazer, newdata = nd, re.form=NA)

## 3. predict grazer biomass for complexity effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$macroalgae <- 0; nd$hard.coral <-0
...
* Complexity has a moderate effect on grazer biomass, which increased from ```r round(10^p.complex[1], 0)``` kg/ha at 0% cover to ```r round(10^p.complex[30], 0)``` kg/ha at ```r round(max(pred$complexity), 0)```  complexity.
...
cat.pred.master<-expand.grid(hard.coral = 0,
                             macroalgae = 0,
                             complexity = 0, fish.dummy = c(0,1), pristine.dummy=c(0,1))
...
```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.width=4}
