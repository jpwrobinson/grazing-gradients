...
p.grazer<-predict(object=m.grazer, newdata = nd, re.form=NA)
...
## NOW PLOT PREDICTIONS
nd<-data.frame(macroalgae = seq(min(h$macroalgae), max(h$macroalgae), length.out=30),
                  hard.coral = 0, complexity = 0, fish.dummy = 0, pristine.dummy=0)

## predict grazer biomass for hard coral effect, holding every other covariate constant
p.grazer<-predict(object=m.grazer, newdata = nd, re.form=NA)

plot(nd$macroalgae, p.grazer, type='l',col=1, axes=F, ylab='Log10 Biomass (kg/ha)', xlab='Macroalgae cover (%)', ylim=c(1, 4))
