...
pairs2(data.frame(h$hard.coral, h$macroalgae, h$rubble, h$substrate, h$complexity, h$management, h$fish.biom), 
...
grazerlog10 ~ fixed(hard.coral + macroalgae + complexity + substrate + rubble + fished + pristine) + random(dataset/reef)
...
m.grazer<-lmer(grazerlog10 ~ hard.coral + macroalgae + rubble + substrate + complexity + fish.biom + fish.dummy + pristine.dummy + ## fixed
          (1 | dataset/reef) , ## random, nested = reefs within datasets
                data = h)
#print(summary(m.grazer),  digits = 2, signif.stars = FALSE)

save(m.grazer, file = '../../results/models/biomass_m.grazers.Rdata')
...
## 5. predict grazer biomass for substrate effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$macroalgae <- 0; nd$hard.coral <-0; nd$rubble <- 0; nd$complexity <- 0
p.substrate<-predict(object=m.grazer, newdata = nd, re.form=NA)
...
plot(cont.pred.master$substrate, p.substrate, type='l',col = cols[1],lwd=2, axes=F, ylab='', xlab='Available substrate', ylim=c(1, 2.5))
...
* Rubble has a moderate effect on grazer biomass, which increased from ```r round(10^p.rubble[1], 0)``` kg/ha at 0% cover to ```r round(10^p.rubble[30], 0)``` kg/ha at ```r round(max(pred$rubble), 0)``` % cover.

* Available substrate has a strong effect on grazer biomass, which declined from ```r round(10^p.substrate[1], 0)``` kg/ha at 0 % available substrate to ```r round(10^p.substrate[30], 0)``` kg/ha at ```r round(max(pred$substrate), 0)```  % available substrate.
...
m.browser<-lmer(browserlog10 ~ hard.coral + macroalgae + complexity + rubble + substrate + fish.biom + fish.dummy + pristine.dummy + ## fixed
          (1 | dataset/reef) , ## random, nested = reefs within datasets
                data = h)
print(summary(m.browser),  digits = 2, signif.stars = FALSE)
save(m.browser, file = '../../results/models/biomass_m.browsers.Rdata')
```

```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.align = 'center', fig.cap='Figure 5: Browser model diagnostics - residuals and fitted values'}
## model diagnostic plots - grazer biomass
par(mfrow=c(2,2), mar=c(5,4,2,2))
...
## 1. predict browser biomass for hard coral effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$macroalgae <- 0; nd$complexity <-0; nd$rubble <- 0; nd$substrate <- 0
...
## 3. predict browser biomass for rubble effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$macroalgae <- 0; nd$hard.coral <-0; nd$complexity <- 0; nd$substrate <- 0
p.rubble<-predict(object=m.browser, newdata = nd, re.form=NA)

## 4. predict browser biomass for complexity effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$macroalgae <- 0; nd$hard.coral <-0; nd$rubble <- 0; nd$substrate <- 0
p.complex<-predict(object=m.browser, newdata = nd, re.form=NA)

## 5. predict browser biomass for substrate effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$macroalgae <- 0; nd$hard.coral <-0; nd$rubble <- 0; nd$complexity <- 0
p.substrate<-predict(object=m.browser, newdata = nd, re.form=NA)

```

```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.align='center', fig.cap = 'Figure 6: Benthic effects on browsers', fig.width=9, fig.height=6}
...
* Rubble has a strong effect on browser biomass, which declined from ```r round(10^p.rubble[1], 0)``` kg/ha at 0% cover to ```r round(10^p.rubble[30], 0)``` kg/ha at ```r round(max(pred$rubble), 0)``` % cover.

* Complexity has the strongest effect on browser biomass, which increased from ```r round(10^p.complex[1], 0)``` kg/ha at 0 complexity to ```r round(10^p.complex[30], 0)``` kg/ha at ```r round(max(pred$complexity), 0)```  complexity.

* Available substrate has a moderate effect on browser biomass, which declined from ```r round(10^p.substrate[1], 0)``` kg/ha at 0 % available substrate to ```r round(10^p.substrate[30], 0)``` kg/ha at ```r round(max(pred$substrate), 0)```  % available substrate.
...
## 1. predict browser biomass for fishing effects
nd<-cat.pred.master
...
## 2. predict browser biomass for fishing biomass gradient
fish.master$p.fish<-predict(object=m.browser, newdata = fish.master, re.form=NA)

```

* Pristine reefs have the highest browser biomass, at ```r round(10^nd$p.fish[nd$categories=='Pristine'], 0)``` kg/ha.
...
```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.align='center', fig.cap='Figure 7: fishing effects on browsers', fig.width=9, fig.height=4}
...
m.scraper<-lmer(scraperlog10 ~ hard.coral + macroalgae + complexity + rubble + substrate + fish.biom + fish.dummy + pristine.dummy + ## fixed
          (1 | dataset/reef) , ## random, nested = reefs within datasets
                data = h)
print(summary(m.scraper),  digits = 2, signif.stars = FALSE)

save(m.scraper, file = '../../results/models/biomass_m.scrapers.Rdata')
```

```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.align = 'center', fig.cap='Figure 8: Scraper model diagnostics - residuals and fitted values'}
...
## 2. predict scraper biomass for macroalgae effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$hard.coral <- 0; nd$complexity <-0; nd$rubble <- 0; nd$substrate <- 0
p.algae<-predict(object=m.scraper, newdata = nd, re.form=NA)

## 3. predict scraper biomass for rubble effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$macroalgae <- 0; nd$hard.coral <-0; nd$complexity <- 0; nd$substrate <- 0
p.rubble<-predict(object=m.scraper, newdata = nd, re.form=NA)

## 4. predict scraper biomass for complexity effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$macroalgae <- 0; nd$hard.coral <-0; nd$rubble <- 0; nd$substrate <- 0
p.complex<-predict(object=m.scraper, newdata = nd, re.form=NA)

## 5. predict scraper biomass for substrate effect, holding every other covariate constant
nd<-cont.pred.master
## set non-focal benthic covariates to 0
nd$macroalgae <- 0; nd$hard.coral <-0; nd$rubble <- 0; nd$complexity <- 0
...
save(cont.pred.master, file = '../../results/models/biomass_m.predictions.Rdata')
```


```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.align='center', fig.cap = 'Figure 9: Benthic effects on scrapers', fig.width=9, fig.height=6}
...
## plot substrate predictions
plot(cont.pred.master$substrate, p.substrate, type='l',col = cols[1],lwd=2, axes=F, ylab='', xlab='Available substrate', ylim=c(2, 3))
axis(1, at = seq(min(h$substrate), max(h$substrate), length.out=5), 
     labels= round(seq(min(pred$substrate), max(pred$substrate), length.out=5), 0))
axis(2)
add_label(0.01, 0.1, font = 2, label ='Substrate')
...
* Hard coral has a moderate effect on scraper biomass, which increased from ```r round(10^p.coral[1], 0)``` kg/ha at 0% cover to ```r round(10^p.coral[30], 0)``` kg/ha at ```r round(max(pred$hard.coral), 0)``` % cover.
...
* Available substrate has a strong effect on scraper biomass, which increased from ```r round(10^p.substrate[1], 0)``` kg/ha at 0 % available substrate to ```r round(10^p.substrate[30], 0)``` kg/ha at ```r round(max(pred$substrate), 0)```  % available substrate.
...
## 1. predict scraper biomass for fishing effects
nd<-cat.pred.master
...
## 2. predict scraper biomass for fishing biomass gradient
fish.master$p.fish<-predict(object=m.scraper, newdata = fish.master, re.form=NA)

save(fish.master, file = '../../results/models/biomass_fishing.effect.predictions.Rdata')
```
...
* Fishable biomass for scrapers ranges from ```r round(range(fish.master$p.fish[fish.master$model == 'grazers']), digits =0) ``` 

<br><br>

```{r, echo=FALSE, include=TRUE, messages=FALSE, fig.align='center', fig.cap='Figure 10: fishing effects on scrapers', fig.width=9, fig.height=4}

## 2. create plot for fishing predictions. 
par(mfrow=c(1,2), mar=c(4,4,1,1), oma=c(0,0,0,0))
plot(c(1,2,3), nd$p.fish, cex=2, col = cols[c(4,5,6)],pch=16, axes=F, ylab='Log10 Biomass (kg/ha)', xlab='', ylim=c(2, 3))
axis(1, at = c(1,2,3), labels= nd$categories)
axis(2)

plot(fish.master$fish.biom, fish.master$p.fish, cex=2, axes=F, ylab='', xlab='Fishable biomass (kg/ha)', ylim=c(2,3), type='l')
axis(1, at = seq(min(h$fish.biom), max(h$fish.biom), length.out=5), 
     labels= comma(round(seq(min(pred$fish.biom), max(pred$fish.biom), length.out=5), 0)))
axis(2)
...
Because covariates were scaled and centered, we can directly compare effect sizes among models and among covariates with different scales (e.g. fishable biomass and percent cover). Pristine reefs - Chagos - are the strongest predictor of biomass, with higher biomass of all 3 FGs. These estimates were highly uncertain, indicative of high site-level variability in biomass. Fishable biomass was consistently strong positive influence on FG biomass, as was rubble, which outweighed hard coral cover as a predictor for all FGs.

Macroalgae had positive effects on browser biomass, and negative effects on grazer and scraper biomass, while complexity was negatively associated with all 3 FGs. On inhabited reefs, fishing had significant negative effects on biomass, though the effect of protection was weaker than that of reef isolation (i.e. remote/pristine Chagos).

```{r, include =T, echo = F, messages=F, fig.caption = 'Figure 11: relative effect of fishing and benthic covariates', fig.width = 12, fig.height = 4, fig.align = 'center'}
...
```{r, include =T, echo = F, messages=F, fig.caption = 'Figure 12. Biomass model results: a. Relative effect sizes of each FG. b. macroalgae. c.available substrate. d. fishable biomass.', fig.width = 12, fig.height = 4, fig.align = 'center'}


#panel 1: relative effect sizes of each FG
...
Our fitted generalised linear mixed model showed differentiating effects for each functional group.

Increasing cover of macroalgae (Fig. 2a) negatively affected biomasses of both herbivore grazers and scrapers, while it slightly elevated that of browsers. Grazer biomass declined from 59 kg/ha at 0% cover to 26 kg/ha at 51 % cover and scraper biomass declined from 265 kg/ha at 0% macroalgae cover to 216 kg/ha at 51 % cover. Browser biomass increased from 47 kg/ha at 0 % macroalgae cover to 66 kg/ha at 51 % cover.

Increasing cover of available substrate (Fig.2b) positively affected herbivore grazers and scrapers, while it negatively affected brower biomass. Grazer biomass increased from 41 kg/ha at 0 % available substrate to 120 kg/ha at 88 % available substrate and scraper biomass increased from 205 kg/ha at 0 % available substrate to 469 kg/ha at 88 % available substrate. Browser biomass declined from 57 kg/ha at 0 % available substrate to 33 kg/ha at 88 % available substrate.

As biomasses of grazers, scrapers, and browsers increased, so did the fishable biomass (Fig. 2c) of each group. This effect was strongly pronounced for browsers, while that for grazers and scrapers was comparable. Grazer biomass increased from 52 kg/ha to 120 kg/ha and scraper biomass increased from 222 kg/ha to 1582 kg/ha across the fishable biomass gradient. Browser biomass increased from 44 kg/ha to 134 kg/ha across the fishable biomass gradient.

Effect sizes (Fig. 2d) expectedly also varied between the three functional groups. The order in which predictors are mentioned is according to the effect size from strongest to weakest. Grazer biomass was positively affected by pristine reefs, available substrate, fishable biomass, structural complexity, and rubble cover. Macroalgae cover, fished reefs, and hard coral cover negatively affected grazer biomass. Scraper biomass was positively affected by pristine reefs, fishable biomass, available substrate, structural complexity, and rubble cover. Fished reefs, macroalgae cover, and hard coral cover negatively affected scraper biomass. Browser biomass was positively affected by pristine reefs, structural complexity, and rubble cover. Fished reefs, Macroalgae cover, and hard coral cover negatively affected browser biomass.
