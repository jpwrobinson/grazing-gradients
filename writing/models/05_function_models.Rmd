...
function ~ fixed(hard.coral + macroalgae + complexity + substrate + rubble + fished + pristine) + random(dataset/reef)
...
#m.grazer<-lmer(log10(cropping.gram.ha) ~ hard.coral + macroalgae + rubble + substrate + complexity + #fish.biom + fish.dummy + pristine.dummy + ## fixed
#          (1 | dataset/reef) , ## random, nested = reefs within datasets
#                data = h)


save(m.grazer, file = '../../results/models/function_m.croppers.Rdata')
...
plot(resid(m.grazer), log10(h$cropping.gram.ha), xlab='Residuals', ylab= 'Observed function', pch=16,col=alpha('black', 0.5), cex.main=0.8)
plot(predict(m.grazer, type='response'), log10(h$cropping.gram.ha), xlab='Predicted function', ylab='Observed function', pch=16, col=alpha('black', 0.5), cex.main=0.8)
```
...
m.scraper<-glmer(scraping ~ hard.coral + macroalgae + rubble + substrate + complexity + fish.biom + fish.dummy + pristine.dummy + ## fixed
          (1 | dataset/reef) , ## random, nested = reefs within datasets
                data = h, family='Gamma'(link='log'))
#m.scraper<-lmer(log10(scraping) ~ hard.coral + macroalgae + rubble + substrate + complexity + fish.biom + #fish.dummy + pristine.dummy + ## fixed
#          (1 | dataset/reef) , ## random, nested = reefs within datasets
#                data = h)


save(m.scraper, file = '../../results/models/function_m.scrapers.Rdata')
