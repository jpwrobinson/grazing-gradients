...
Aim is to build a predictive model that estimates the bite rate of each fish in the UVC dataset, given its species and, for scrapers, its size. Model distributions will be set according to the bite rate distributions, which are all left-skewed. When data are continuous and not normally distributed (Figure 3; grazers and scrapers), we'll use the flexible Gamma distribution.

<br><br>

```{r,echo=FALSE, eval = FALSE, messages = FALSE, fig.align='center', fig.cap='Figure 3: histogram of bite rates by functional group', fig.width=7, fig.height = 4}
...
$log(\mu) = X + species_i + family_j + genus_k$
...
p$col<-grazers$Genus[match(p$class, grazers$sp)]
p$col[p$preds == 'Genus']<-grazers$Genus[match(p$class[p$preds == 'Genus'], grazers$Genus)]
```

```{r,echo=FALSE,fig.align='center', fig.cap = 'Figure 4: Grazer bite rates by genera and species. Points are median posterior predictions, with 95% certainty intervals.', fig.width= 8, fig.height = 5}
...
Scraper function is removal of dead coral substrate, endolithic material, and detritus. We infer this function from the total bite rate of the scraper community, and accounting for changes in bite scar volume with size. Thus, the predictive model estimates bite rates by species, families, and size. We can use a hierarchical Bayesian model to predict new levels (i.e. species), based on their genus.

<br><br>

$bite = Gamma(\mu, \theta)$

$log(\mu) = X + B*size + species_i + family_j + genus_k$

<br><br>

where X is the intercept, for ```r uniques(bite$sp[bite$FG == 'Herbivore Scraper'])``` species in ```r uniques(bite$Genus[bite$FG == 'Herbivore Scraper'])``` genera
...
scrape.m<-map2stan(
        ...
        data=scrapers, iter = 3000, chains =1)
...
For scrapers, there is strong variation in bite rates among species, but less among genera. This suggests that predicting unobserved species may be easier for this FG, though scrapers were well covered for bite rate data. Scarus have the highest growth rates, with some species exceeding 40 bites per minute (S. spinus, globiceps, and viridifucatus). Correcting these rates to volumes grazed is important, so we shouldn't interpret these patterns yet.
...
## Predictions for species, with genus effect
d.pred <-data.frame(sp = unique(scrapers$sp), TL = mean(scrapers$TL))
d.pred$Genus<-scrapers$Genus[match(d.pred$sp, scrapers$sp)]
link.obs.sp<-link(scrape.m, n = 1000, data = as.list(d.pred))
pred.mean<-data.frame(median = apply(link.obs.sp, 2, median))
pred.PI<-apply(link.obs.sp, 2, PI, prob = 0.95)
pred.mean$upper<-pred.PI[2,]
pred.mean$lower<-pred.PI[1,]
...
## Predictions for genus, removing species effect
d.pred <- data.frame(Genus = unique(scrapers$Genus), sp = 'NA', TL = mean(scrapers$TL))
a.sp.zero = matrix(0, 1000, 28)
link.obs.genus<-link(scrape.m, n = 1000, data = as.list(d.pred), replace= list(X1= a.sp.zero))
pred.mean.genus<-data.frame(median = apply(link.obs.genus, 2, median))
pred.PI<-apply(link.obs.genus, 2, PI, prob = 0.95)
pred.mean.genus$upper<-pred.PI[2,]
pred.mean.genus$lower<-pred.PI[1,]
...
```{r, echo=F, fig.align='center', fig.cap = 'Figure 5: Scraper bite rates by genera and species. Points are median posterior predictions, with 95% certainty intervals.', fig.width= 8, fig.height = 5}
...
Size effects were also relative strong, with bite rates decreasing by 50% from the smallest to largest individuals. High uncertainty in this relationship suggests there are important species-specific size differences here, but we have not accounted for these.

<br><br>

```{r, echo=FALSE, eval = TRUE, messages = FALSE, include=F}
## size effects
d.pred <- data.frame(Genus = 'NA', sp = 'NA', TL = seq(min(scrapers$TL), max(scrapers$TL), length.out =30))
a.sp.zero = matrix(0, 1000, 28); a.genus.zero = matrix(0, 1000, 4)
link.obs.size<-link(scrape.m, n = 1000, data = as.list(d.pred), replace= list(X1= a.sp.zero, X2 = a.genus.zero))
...
```{r,echo=F, fig.align='center', fig.cap = 'Figure 6: Scraper bite rates by size. Lines are median posterior predictions with 95% certainty intervals.', fig.width= 8, fig.height = 4}
...
Browser function is removal/consumption of macroalgae. We infer this function from the total bite rate of the browser community, and assume all bites are the same size. Thus, the predictive model estimates bite rates by 2 genera. We can use a hierarchical Bayesian model to predict new levels (i.e. species), based on their genus. Note there are only 2 species and 2 genera, so only fitting a genera model, and we are missing observations of any siganid species (these are high biomass on macroalgal reefs).
...
where X is the intercept, for ```r uniques(bite$sp[bite$FG == 'Herbivore Browser'])``` species in ```r uniques(bite$Genus[bite$FG == 'Herbivore Browser'])``` genera
...
browse.m<-map2stan(
        alist(
          ...
          X2[Genus] ~ dnorm(0, sigmar2),
          a ~ dnorm(11.98, 10),
          c(sigma, sigmar2) ~ dcauchy(0, 1)
        ),
        data=browsers, iter = 3000, chains =1)
...
d.pred <- data.frame(Genus = unique(browsers$Genus))
...
```{r,echo=F, fig.align='center', fig.cap = 'Figure 7: Browser bite rates by genera and species. Points are median posterior predictions, with 95% certainty intervals.', fig.width= 8, fig.height = 5}
