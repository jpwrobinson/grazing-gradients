...
rsquared(m.graze) ## marginal = 0.92, conditional = 0.966
...
h<-h[!is.na(h$evenness),]

## new decoupling model to account for div and size structure
m.graze2<-lmer(cropping.gram.ha ~ biom.scaled + mean.mass.scaled + sp.richness.scaled + evenness.scaled +
          (1 | dataset/reef), h)
summary(m.graze2)
rsquared(m.graze2) ## maringal = 0.9676; conditional = 0.9844
...
nd.rich$pred<-predict(m.graze2, newdata=nd.rich, re.form=NA)
...
nd.size$pred<-predict(m.graze2, newdata=nd.size, re.form=NA)


partials<-visreg::visreg(m.graze2, 'sp.richness.scaled')
...
richness.points<-data.frame(x=h$sp.richness, 
          y = partials$res$visregRes, dataset=partials$res$dataset)

partials<-visreg::visreg(m.graze2, 'mean.mass.scaled')
...
g1<-ggplot(richness.fit, aes(richness, fit)) + geom_line() + 
    ...
    labs(y = 'Partial effect on algae removed', x = 'Species richness') +
...
pdf(file='figures/explore/test_grazer_resids.pdf', height = 4, width=9)
