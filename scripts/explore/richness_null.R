...
## transpose
com.mat<-t(com.mat)
colnames(com.mat)<-rows$unique.transect
...
diversity.preds$estimated<-richness$mean[match(diversity.preds$unique.id, richness$unique.id)]
diversity.preds$region<-pred$dataset[match(diversity.preds$unique.id, pred$unique.id)]

ggplot(diversity.preds, aes(richness, estimated, col=region)) + geom_point()
